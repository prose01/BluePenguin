...
export class ColourPaletteComponent implements AfterViewInit, OnChanges {
  ...
  draw() {
    if (!this.ctx) {
      this.ctx = this.colourcanvas.nativeElement.getContext('2d');
    }
    const width = this.colourcanvas.nativeElement.width;
    const height = this.colourcanvas.nativeElement.height;
